...
transformed data{
  vector[All] alpha_raw;
  for(idx in 1:All) {
    alpha_raw[idx] = log(alpha[idx]/(1-alpha[idx]));
  }
}
...
model {
  ...
  for(i in 1:All) {
    mu =  beta[1]+
          ...
          r_group[groupID[i]];
    alpha_raw[i] ~ normal(mu, sigma_e);
  }
}

generated quantities {
  vector[test_size] alpha_pred_25;
  vector[test_size] alpha_pred_50;
  vector[test_size] alpha_pred_78;
