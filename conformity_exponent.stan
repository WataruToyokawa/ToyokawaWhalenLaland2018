...
model {
  ...
  for(i in 1:All) {
    ...
    conformity_exponent[i] ~ normal(mu, sigma);
  }
}

generated quantities {
  vector[test_size] conformity_exponent_pred_25;
  vector[test_size] conformity_exponent_pred_50;
  vector[test_size] conformity_exponent_pred_78;
  for(gs in 1:test_size){
    conformity_exponent_pred_25[gs] = 
    normal_rng(
        beta[1]+
        ...
        beta[11]*0*0.5
      , sigma_e + gamma*0);
    conformity_exponent_pred_50[gs] = 
    normal_rng(
        beta[1]+
        beta[2]*test_group_size[gs] + 
        ...
        beta[11]*0*0.5
      , sigma_e + gamma*0.5);
    conformity_exponent_pred_78[gs] = 
    normal_rng(
        beta[1]+
        beta[2]*test_group_size[gs] + 
        beta[3]*1 +//difficulty
