...
functions {
  real UNC4_lpmf(int Y, real soc_raw, real soc_reduction, int trial, vector q, vector q_f, vector f) {
...
model {
  ...
  for(idx in 1:All) {
    if(Y[idx] > 0) {
      target += 
        //1/log(All) *
        UNC4_lpmf(Y[idx] | soc_raw[sub[idx]], soc_reduction[sub[idx]], trial[idx], q[sub[idx]][,trial[idx]], q_f[sub[idx]][,trial[idx]], f[sub[idx]][,trial[idx]] );
    }
  }
}
